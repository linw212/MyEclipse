...
function executeCallBackFun(functionStr, options) {
    if (void 0 != functionStr) {
        for (var handlerBeforeArr = functionStr.split("|"), handlerBeforeParamsArr = handlerBeforeArr[1].split(","), handlerBeforeParams = "", h = 0; h < handlerBeforeParamsArr.length; h++)
            handlerBeforeParams += handlerBeforeParamsArr[h].indexOf("options.") > -1 ? handlerBeforeParamsArr[h] + "," : '"' + handlerBeforeParamsArr[h] + '",';
...
function isMultiArr(a) {
    ...
    for (var b = 0; b < a.length; b++)
        return a[b]instanceof Array
}
function isMultiObj(a) {
    if (!a)
        return !1;
    for (var b = 0; b < a.length; b++)
        return "object" == typeof a[b]
}
...
function addTab(a) {
    ...
    if (c.tabs("exists", d.title))
        c.tabs("select", d.title);
    else {
        var e = $.cookie("menuClickTime")
          , f = (new Date).getTime();
        f - e >= 1e3 ? ($.cookie("menuClickTime", (new Date).getTime()),
        c.tabs("myAdd", d)) : $.messager.show({
            title: "温馨提示",
            msg: "操作过快，请稍后重试！"
        })
    }
...
function bindMenuClickEvent(a, b) {
    ...
    b.dialog.width = b.dialog.width ? b.dialog.width : 700,
    b.dialog.height = b.dialog.height ? b.dialog.height : 450,
    b = $.extend({}, b, e)) : "openTab" == b.event ? (e = {
        iconCls: "fa fa-th"
    },
...
function openDialogAndloadDataByParentGrid(a) {
    var b = ""
      , c = "";
    "object" == typeof a.parentGrid && (b = a.parentGrid.unselectedMsg,
    c = a.parentGrid.param,
    "datagrid" == a.parentGrid.type || "treegrid" == a.parentGrid.type);
    var d = getSelectedRowData(a.parentGrid.type, a.parentGrid.id);
    if (!d)
        return void $.messager.alert(topJUI.language.message.title.operationTips, a.parentGrid.unselectedMsg || topJUI.language.message.msg.selectParentGrid, topJUI.language.message.icon.warning);
    if (!a.dialog.beforeOpenCheckUrl || beforeOpenCheck(replaceUrlParamValueByBrace(a.dialog.beforeOpenCheckUrl, d))) {
        ...
        if (a.dialog.href.indexOf("{") != -1) {
            if (a.dialog.href.indexOf("{parent.") != -1 && (g = replaceUrlParamValueByBrace(appendSourceUrlParam(f), d, "parent")),
            g.indexOf("{") != -1) {
...
function openDialogAndloadDataByUrl(a) {
    ...
    if (!b)
        return void $.messager.alert(topJUI.language.message.title.operationTips, topJUI.language.message.msg.selectSelfGrid, topJUI.language.message.icon.warning);
...
function addChildHandler(a) {
    ...
    if (b) {
        ...
        setTimeout(function() {
            ...
            var c = {};
            if (a.gridParam)
                for (var d = a.gridParam.split(","), f = 0; f < d.length; f++)
                    c[d[f]] = b[d[f]];
            c.puuid = b.uuid,
            e.form("load", c)
        }, 500)
    } else
        $.messager.alert(topJUI.language.message.title.operationTips, topJUI.language.message.msg.selectParentGrid, topJUI.language.message.icon.warning)
}
...
function getRowsDataBySelected2(a, b) {
    var c, d;
    return "object" == typeof a.parentGrid ? (d = a.parentGrid.id,
    "datagrid" == a.parentGrid.type ? c = b ? $("#" + d).datagrid("getSelections") : $("#" + d).datagrid("getSelected") : "treegrid" == a.parentGrid.type && (c = b ? $("#" + d).treegrid("getSelections") : $("#" + d).treegrid("getSelected"))) : "object" == typeof a.grid && (d = a.grid.id,
    "datagrid" == a.grid.type ? c = b ? $("#" + d).datagrid("getSelections") : $("#" + d).datagrid("getSelected") : "treegrid" == a.grid.type && (c = b ? $("#" + d).treegrid("getSelections") : $("#" + d).treegrid("getSelected"))),
    c
}
...
function refreshGrid(a, b, c) {
    ...
    if ("datagrid" == a)
        ...
        for (var e = d[a]("getRoot"), f = d[a]("getData", e), g = 0; g < f.length; g++)
            d[a]("reload", f[g].id);
    d[a]("unselectAll"),
    d[a]("uncheckAll")
}
function loadGridByQueryParm(a, b, c) {
    "datagrid" == a ? ($("#" + b).datagrid("load", c),
    $("#" + b).datagrid("unselectAll")) : "treegrid" == a && ($("#" + b).treegrid("load", c),
    $("#" + b).treegrid("unselectAll"))
}
function doAjaxHandler(a) {
    ...
    b.url.indexOf("{parent") != -1) {
        var d = getSelectedRowData(b.parentGrid.type, b.parentGrid.id);
        if (!d)
            return void $.messager.alert(topJUI.language.message.title.operationTips, topJUI.language.message.msg.selectParentGrid, topJUI.language.message.icon.warning);
...
function requestHandler(a) {
    var b = $.data(a, "menubutton").options;
    if (b.url = appendSourceUrlParam(b.url),
    "object" == typeof b.grid) {
    ...
    if ("object" == typeof b.grid) {
        ...
        $("#submitImportForm").linkbutton({
            ...
            onClick: function() {
                ...
                d.url = b.url;
                var e = doAjax(d);
                200 == e.statusCode && ($("#" + b.dialog.id).dialog("close"),
                refreshGrid(b.grid.type, b.grid.id)),
                showMessage(e)
            }
        }),
...
function uploadHandler(a) {
    ...
    if ("object" == typeof b.grid) {
            ...
            c.render({
                ...
                choose: function(a) {
                    ...
                    if (200 != a.statusCode)
                        ...
                        for (var d = upfileGrid.datagrid("getRows"), e = 0; e < d.length; e++)
        ...
        var g = function() {
            for (var a = upfileGrid.datagrid("getSelections"), b = [], c = 0; c < a.length; c++)
                b.push(a[c]);
    ...
    if ("object" == typeof b.grid)
        ...
        "datagrid" == b.grid.type) {
            ...
            for (var m = 0; m < h.length; m++)
                i = $("#" + e).treegrid("getColumnOption", h[m]),
                j.push(i.title),
                1 == i.hidden || 1 == i.checkbox ? k.push(!0) : k.push(!1)
        }
...
function doAjax(a) {
    ...
    $.ajax({
        ...
        success: function(c, d, e) {
            ...
            1 == c.statusCode || 100 == c.statusCode || 200 == c.statusCode ? (b.statusCode = 200,
            b.title = c.title,
            b.message = c.message) : 1 == c ? (b.statusCode = 200,
            b.title = "操作提示",
            b.message = "操作成功！") : (b.statusCode = 300,
            b.title = c.title,
            b.message = c.message)
        }
    }),
...
function setDialogHrefKeyValue(a, b, c) {
    ...
    for (i = 0; i < d.length; i++)
        f.indexOf("?") > 0 ? f.indexOf(d[i] + "=" + e[i]) == -1 && (g += "&" + d[i] + "=" + e[i]) : 0 == i ? g = "?" + d[i] + "=" + e[i] : g += "&" + d[i] + "=" + e[i];
...
function showMessage(a) {
    var b = {}
      , c = "";
    "object" == typeof a ? (c = a.statusCode,
    void 0 == a.icon && (a.icon = topJUI.language.message.icon.info),
    b = {
    ...
    !(1 != c && 100 != c && 200 != c || 1 != c && 200 != c) ? (b.timeout = 1e3,
    $.messager.show(b)) : $.messager.alert(b)
}
...
function getMultiRowsFieldValue(a, b) {
    for (var c = [], d = 0; d < a.length; d++)
        topJUI.config.singleQuotesParam ? c.push("'" + a[d][b] + "'") : c.push(a[d][b]);
    return c.join(",")
}
!function(a) {
    a.easyui = {
        ...
        forEach: function(a, b, c) {
            ...
            for (; d.length; ) {
                ...
                if (0 == c(f))
                    return;
                if (b && f.children)
                    for (var e = f.children.length - 1; e >= 0; e--)
                        d.unshift(f.children[e])
            }
        }
    },
    a.parser = {
        ...
        parseValue: function(b, c, d, e) {
            ...
            return "%" == g ? (f = parseFloat(f.substr(0, f.length - 1)),
            f = b.toLowerCase().indexOf("width") >= 0 ? Math.floor((d.width() - e) * f / 100) : Math.floor((d.height() - e) * f / 100)) : f = parseInt(f) || void 0,
            f
        },
    ...
    a.fn._size = function(b, c) {
        function d(b, c, d) {
            ...
            var e = a(b)[0]
              , f = c[0]
              , g = f.fcount || 0;
            return d ? (e.fitted || (e.fitted = !0,
            f.fcount = g + 1,
            a(f).addClass("panel-noscroll"),
            ...
            f.fcount = g - 1,
            0 == f.fcount && (a(f).removeClass("panel-noscroll"),
            "BODY" == f.tagName && a("html").removeClass("panel-fit"))),
            !1)
        }
        function e(b, c, d, e) {
            var f = a(b)
        ...
        function f(b, c, d) {
            ...
            var f = a(b);
            if (void 0 == d) {
                if (d = parseInt(b.style[c]),
                isNaN(d))
                    return;
                return a._boxModel && (d += e()),
                d
            }
            "" === d ? f.css(c, "") : (a._boxModel && (d -= e(),
            d < 0 && (d = 0)),
            f.css(c, d + "px"))
        }
...
function(a) {
    ...
    function f(b) {
        ...
        function d() {
            ...
            return f.droppables.each(function() {
                ...
                if (!e.droppable("options").disabled) {
                    ...
                    a(this).triggerHandler("_drop", [b.data.target]),
                    c(),
                    d = !0,
                    this.entered = !1,
                    !1) : void 0
        ...
        var f = a.data(b.data.target, "draggable")
          , h = f.proxy
          , i = f.options;
        if (i.onEndDrag.call(b.data.target, b),
        i.revert)
            if (1 == d())
                a(b.data.target).css({
                    position: b.data.startPosition,
                ...
                var j, k;
                h.parent()[0] == document.body ? (j = b.data.startX - b.data.offsetWidth,
        ...
        return "string" == typeof b ? a.fn.draggable.methods[b](this, c) : this.each(function() {
            ...
            g.disabled ? void a(this).css("cursor", "") : void i.unbind(".draggable").bind("mousemove.draggable", {
                ...
                if (0 != c(b)) {
                    ...
                    if (0 != j.onBeforeDrag.call(b.data.target, b))
                        return a(document).bind("mousedown.draggable", b.data, d),
...
function(a) {
    ...
    function g(b) {
        ...
        a.inArray("all", j) >= 0 || a.inArray(e, j) >= 0)
            return e;
    ...
    a.fn.resizable = function(b, c) {
        return "string" == typeof b ? a.fn.resizable.methods[b](this, c) : this.each(function() {
            ...
            c = a.extend(h.options, b || {})) : (c = a.extend({}, a.fn.resizable.defaults, a.fn.resizable.parseOptions(this), b || {}),
...
function(a) {
    ...
    function c(b) {
        ...
        c.iconCls && (a('<span class="l-btn-icon">&nbsp;</span>').addClass(c.iconCls).appendTo(h),
        h.addClass("l-btn-icon-" + c.iconAlign)),
        f.unbind(".linkbutton").bind("focus.linkbutton", function() {
            ...
            c.disabled || (c.toggle && (c.selected ? a(this).linkbutton("unselect") : a(this).linkbutton("select")),
            c.onClick.call(this))
        }),
        d(b, c.selected),
        e(b, c.disabled)
    ...
    a.fn.linkbutton = function(d, e) {
        ...
        this.each(function() {
            ...
            a(this).bind("_resize", function(c, d) {
...
function(a) {
    ...
    function c(b) {
        ...
        a(b).unbind().bind("mouseover", function(b) {
            ...
            e.length && (K(b, e[0]),
            c.onDblClick.call(b, G(b, e[0])),
            d.stopPropagation())
        }).bind("contextmenu", function(d) {
            var e = a(d.target).closest("div.tree-node");
            e.length && (c.onContextMenu.call(b, d, G(b, e[0])),
            d.stopPropagation())
        })
    }
    function d(b) {
    ...
    function e(b) {
        ...
        i.find("div.tree-node").draggable({
            ...
            onBeforeDrag: function(c) {
                if (0 == h.onBeforeDrag.call(b, G(b, this)))
                    return !1;
            ...
            onStartDrag: function(c) {
                ...
                var d = G(b, this);
                void 0 == d.id && (d.id = "easyui_tree_node_id_temp",
                y(b, d)),
    ...
    function f(b, c, d, e) {
        ...
        if (j.checkbox) {
            ...
            if (k.checkState) {
                ...
                if (void 0 == d && (l.hasClass("tree-checkbox1") ? d = !1 : l.hasClass("tree-checkbox0") ? d = !0 : (void 0 == k._checked && (k._checked = a(c).find(".tree-checkbox").hasClass("tree-checkbox1")),
                d = !k._checked)),
                k._checked = d,
                d) {
                    ...
                    return;
                (e || 0 != j.onBeforeCheck.call(b, k, d)) && (j.cascadeCheck ? (g(b, k, d),
                i(b, k)) : h(b, k, d ? "1" : "0"),
                e || j.onCheck.call(b, k, d))
            }
        }
    }
    function g(b, c, d) {
        ...
        e.deepCheck)
            ...
            a.easyui.forEach(c.children || [], !0, function(a) {
                a.hidden || (h(b, a, f),
                a.children && a.children.length && g.push(a))
            });
    ...
    function j(b) {
        ...
        if (a.easyui.forEach(b.children || [], !1, function(a) {
            a.checkState && (e++,
            "checked" == a.checkState ? d++ : "unchecked" == a.checkState && c++)
        }),
        0 != e) {
            var f = 0;
    ...
    function k(b, c) {
        ...
        if (d.checkbox) {
            ...
            if (d.view.hasCheckbox(b, h))
                if (g.length || (h.checkState = h.checkState || "unchecked",
                a('<span class="tree-checkbox"></span>').insertBefore(e.find(".tree-title"))),
                "checked" == h.checkState)
                    f(b, c, !0, !0);
                else if ("unchecked" == h.checkState)
                    f(b, c, !1, !0);
                else {
                    var k = j(h);
    ...
    function l(b, c, d, g, h) {
        ...
        g ? l ? l.children ? l.children = l.children.concat(d) : l.children = d : i.data = i.data.concat(d) : (l ? l.children = d : i.data = d,
        a(c).empty()),
        j.view.render.call(j.view, b, c, d),
        j.dnd && e(b),
        l && y(b, l);
        for (var n = 0; n < i.tmpIds.length; n++)
            f(b, a("#" + i.tmpIds[n])[0], !0, !0);
        i.tmpIds = [],
        setTimeout(function() {
            m(b, b)
        }, 0),
    ...
    function M(b, c) {
        ...
        if (0 != d.onBeforeEdit.call(b, e)) {
            ...
            h.bind("click", function(a) {
                ...
                return 13 == a.keyCode ? (N(b, c),
                !1) : 27 == a.keyCode ? (O(b, c),
                !1) : void 0
    ...
    function P(b, c) {
        function d(c) {
            for (var d = a(b).tree("getParent", a("#" + c)[0]); d; )
                a(d.target).removeClass("tree-node-hidden"),
                d.hidden = !1,
        ...
        a.easyui.forEach(e.data, !0, function(d) {
            f.filter.call(b, c, d) ? (a("#" + d.domId).removeClass("tree-node-hidden"),
            g[d.domId] = 1,
      ...
      , R = {
        render: function(b, c, d) {
            function e(c, d) {
                for (var h = [], j = 0; j < d.length; j++) {
                    ...
                    if ("closed" == k.state ? (h.push('<span class="tree-hit tree-collapsed"></span>'),
                    h.push('<span class="tree-icon tree-folder ' + (k.iconCls ? k.iconCls : "") + '"></span>')) : k.children && k.children.length ? (h.push('<span class="tree-hit tree-expanded"></span>'),
                    h.push('<span class="tree-icon tree-folder tree-folder-open ' + (k.iconCls ? k.iconCls : "") + '"></span>')) : (h.push('<span class="tree-indent"></span>'),
                    h.push('<span class="tree-icon tree-file ' + (k.iconCls ? k.iconCls : "") + '"></span>')),
                    this.hasCheckbox(b, k)) {
                        var m = 0;
                        i && "checked" == i.checkState && g.cascadeCheck ? (m = 1,
                        k.checked = !0) : k.checked && a.easyui.addArrayItem(f.tmpIds, k.domId),
                        k.checkState = m ? "checked" : "unchecked",
                        h.push('<span class="tree-checkbox tree-checkbox' + m + '"></span>')
                    } else
                        k.checkState = void 0,
                        k.checked = void 0;
                    if (h.push('<span class="tree-title">' + g.formatter.call(b, k) + "</span>"),
                    h.push("</div>"),
        ...
        hasCheckbox: function(b, c) {
            var d = a.data(b, "tree")
              , e = d.options;
            if (e.checkbox) {
                if (a.isFunction(e.checkbox))
                    return !!e.checkbox.call(b, c);
                if (!e.onlyLeafCheck)
                    return !0;
                if (!("open" != c.state || c.children && c.children.length))
                    return !0
            }
            return !1
        }
...
function(a) {
    ...
    function e(b) {
        function c(c) {
            ...
            var d, g, h = a.isFunction(e.deltaX) ? e.deltaX.call(b, e.position) : e.deltaX, i = a.isFunction(e.deltaY) ? e.deltaY.call(b, e.position) : e.deltaY;
            if (e.trackMouse)
                j = a(),
                d = e.trackMouseX + h,
                g = e.trackMouseY + i;
        ...
        if (d && d.tip) {
            ...
            if (a(b).is(":visible"))
                if (g = c(e.position),
                "top" == e.position && g.top < 0 ? g = c("bottom") : "bottom" == e.position && g.top + f._outerHeight() > a(window)._outerHeight() + a(document).scrollTop() && (g = c("top")),
                g.left < 0)
                    "left" == e.position ? g = c("right") : (a(b).tooltip("arrow").css("left", f._outerWidth() / 2 + g.left),
                    g.left = 0);
...
function($) {
    ...
    function setSize(a, b) {
        ...
        isNaN(parseInt(d.height))) {
            ...
            g._outerHeight(e.height() - f._outerHeight() - h._outerHeight());
        e.css({
            height: i ? void 0 : "",
            minHeight: "",
            maxHeight: "",
    ...
    function movePanel(a, b) {
        var c = $.data(a, "panel")
          , d = c.options
          , e = c.panel;
        b && (null != b.left && (d.left = b.left),
        null != b.top && (d.top = b.top)),
        e.css({
            left: d.left,
            top: d.top
        }),
    ...
    function createPanel(target) {
        function _addHeader() {
            if (opts.noheader || !opts.title && !opts.header)
                ...
                if (opts.header)
                    ...
                    opts.iconCls && (htitle.addClass("panel-with-icon"),
        ...
        function _addFooter() {
            opts.footer ? ($(opts.footer).addClass("panel-footer").appendTo(panel),
            $(target).addClass("panel-body-nobottom")) : (panel.children(".panel-footer").remove(),
        ...
        opts.content && ($(target).panel("clear"),
    ...
    function loadData(a, b) {
        var c = $.data(a, "panel")
          , d = c.options;
        if (e && (d.queryParams = b),
        d.href && (!c.isLoaded || !d.cache)) {
            var e = $.extend({}, d.queryParams);
            if (0 == d.onBeforeLoad.call(a, e))
                return;
            c.isLoaded = !1,
            d.loadingMessage && ($(a).panel("clear"),
            $(a).html($('<div class="panel-loading"></div>').html(d.loadingMessage))),
    ...
    function openPanel(a, b) {
        function c() {
            ...
            d.onOpen.call(a),
            1 == d.maximized && (d.maximized = !1,
            maximizePanel(a)),
            1 == d.collapsed && (d.collapsed = !1,
            collapsePanel(a)),
            d.collapsed || (loadData(a),
            doLayout(a))
        }
        var d = $.data(a, "panel").options
          , e = $.data(a, "panel").panel;
        if (1 == b || 0 != d.onBeforeOpen.call(a))
            if (e.stop(!0, !0),
            $.isFunction(d.openAnimation))
                d.openAnimation.call(a, c);
            else
                switch (d.openAnimation) {
                case "slide":
    ...
    function closePanel(a, b) {
        ...
        if (1 == b || 0 != e.onBeforeClose.call(a))
            ...
            f.stop(!0, !0),
            f._size("unfit"),
            $.isFunction(e.closeAnimation))
                e.closeAnimation.call(a, c);
            else
                switch (e.closeAnimation) {
                case "slide":
    ...
    function collapsePanel(a, b) {
        ...
        1 != d.collapsed && (f.stop(!0, !0),
        0 != d.onBeforeCollapse.call(a) && (h.addClass("panel-tool-expand"),
        1 == b ? "left" == d.halign || "right" == d.halign ? e.animate({
            width: g._outerWidth() + e.children(".panel-footer")._outerWidth()
        }, function() {
            c()
        }) : f.slideUp("normal", function() {
            c()
        }) : ("left" != d.halign && "right" != d.halign || e._outerWidth(g._outerWidth() + e.children(".panel-footer")._outerWidth()),
        c())))
    }
    function expandPanel(a, b) {
        ...
        0 != d.collapsed && (f.stop(!0, !0),
        0 != d.onBeforeExpand.call(a) && (g.removeClass("panel-tool-expand"),
        1 == b ? "left" == d.halign || "right" == d.halign ? (f.show(),
        e.animate({
            ...
            c()
        }) : ("left" != d.halign && "right" != d.halign || e.css("width", d.panelCssWidth),
        c())))
    }
...
function(a) {
    ...
    function c(c, d) {
        ...
        if (e.inline) {
            ...
            e.left = Math.ceil((a(window)._outerWidth() - g) / 2 + a(document).scrollLeft());
        d && b(c)
    }
    function d(c, d) {
        var e = a.data(c, "window").options
        ...
        if (e.inline) {
            ...
            e.top = Math.ceil((a(window)._outerHeight() - g) / 2 + a(document).scrollTop());
        d && b(c)
    }
    function e(e) {
        var f = a.data(e, "window")
          ...
          , h = a(e).panel(a.extend({}, f.options, {
            ...
            onBeforeDestroy: function() {
                return 0 != g.onBeforeDestroy.call(e) && (f.shadow && f.shadow.remove(),
    ...
    function f(b, c, d, e) {
        ...
        if (!h.constrain)
            return {};
        if (a.isFunction(h.constrain))
            return h.constrain.call(f, b, c, d, e);
...
function($) {
    function buildDialog(target) {
        ...
        $(target).window($.extend({}, opts, {
            onResize: function(a, b) {
                opts.inited && (setContentSize(this),
                opts.onResize.call(this, a, b))
            }
        }));
    ...
    function setContentSize(a, b) {
        ...
        if (isNaN(parseInt(d.height))) {
            ...
            c._outerHeight(c._outerHeight() - h);
    ...
    function b(b, c) {
        function d(a, b) {
            for (var c = 0, d = 0; d < g.length; d++) {
                var e = g[d];
                if (i)
                    var f = e.panel("header")._outerWidth(j);
                else
                    var f = e.panel("header")._outerHeight(j);
...
function($) {
    ...
    function setSize(a, b) {
        ...
        if (e.doSize) {
            ...
            var g = f.children("div.tabs-header")
              ...
              , j = i.find(".tabs");
            if (j.children("li").removeClass("tabs-first tabs-last"),
            j.children("li:first").addClass("tabs-first"),
            j.children("li:last").addClass("tabs-last"),
            "left" == e.tabPosition || "right" == e.tabPosition ? (g._outerWidth(e.showHeader ? e.headerWidth : 0),
            h._outerWidth(f.width() - g.outerWidth()),
            g.add(h)._size("height", isNaN(parseInt(e.height)) ? "" : f.height()),
            i._outerWidth(g.width()),
            ...
            g._outerWidth(f.width()).css("height", ""),
            e.showHeader ? (g.css("background-color", ""),
            i.css("height", "")) : (g.css("background-color", "transparent"),
            g._outerHeight(0),
            i._outerHeight(0)),
            j._outerHeight(e.tabHeight).css("width", ""),
            j._outerHeight(j.outerHeight() - j.height() - 1 + e.tabHeight).css("width", ""),
            h._size("height", isNaN(parseInt(e.height)) ? "" : f.height() - g.outerHeight()),
            h._size("width", f.width())),
            d.tabs.length) {
    ...
    function createTab(a, b, c) {
        ...
        c || (c = $("<div></div>")),
        b.index >= e.length ? (h.appendTo(f),
        c.appendTo(g),
        e.push(c)) : (h.insertBefore(f.children("li:eq(" + b.index + ")")),
        c.insertBefore(g.children("div.panel:eq(" + b.index + ")")),
        e.splice(b.index, 0, c)),
        c.panel($.extend({}, b, {
            tab: h,
            border: !1,
            noheader: !0,
            ...
            onBeforeOpen: function() {
                if (b.onBeforeOpen && 0 == b.onBeforeOpen.call(this))
                    return !1;
                var c = $(a).tabs("getSelected");
                if (c) {
                    if (c[0] == this)
                        return setSelectedSize(a),
                        !1;
                    if ($(a).tabs("unselect", getTabIndex(a, c)),
                    c = $(a).tabs("getSelected"))
                        return !1
                }
                var d = $(this).panel("options");
    ...
    function updateTab(a, b) {
        ...
        "all" != b.type && "body" != b.type || d.panel(),
        "all" == b.type || "header" == b.type) {
            var g = e.tab;
            if (e.header)
                g.find(".tabs-inner").html($(e.header));
            else {
                ...
                e.closable ? (h.addClass("tabs-closable"),
                $('<a href="javascript:;" class="tabs-close"></a>').appendTo(g)) : h.removeClass("tabs-closable"),
                e.iconCls ? (h.addClass("tabs-with-icon"),
                i.addClass(e.iconCls)) : h.removeClass("tabs-with-icon"),
                e.tools) {
                    var j = g.find("span.tabs-p-tool");
                    if (!j.length)
                        var j = $('<span class="tabs-p-tool"></span>').insertAfter(g.find("a.tabs-inner"));
                    if ($.isArray(e.tools)) {
                        j.empty();
    ...
    function getTab(a, b, c) {
        var d = $.data(a, "tabs").tabs
          , e = null;
        if ("number" == typeof b)
            b >= 0 && b < d.length && (e = d[b],
            c && d.splice(b, 1));
        else {
            for (var f = $("<span></span>"), g = 0; g < d.length; g++) {
                var h = d[g];
                if (f.html(h.panel("options").title),
                f.text() == b) {
                    e = h,
                    c && d.splice(g, 1);
                    break
    ...
    function selectTab(a, b) {
        var c = getTab(a, b);
        c && !c.is(":visible") && (stopAnimate(a),
        c.panel("options").disabled || c.panel("open"))
    }
...
function(a) {
        ...
        if (!i[l].length) {
                ...
                onOpen: function() {
                    ...
                    if (d[l]) {
                        var e = "layout-button-" + d[l]
                          , g = b.children("a." + e);
                        g.length || (g = a('<a href="javascript:;"></a>').addClass(e).appendTo(b),
                        g.bind("click", {
                            dir: l
                        }, function(a) {
            ...
            p.resizable(a.extend({}, {
                ...
                onStartResize: function(b) {
                    ...
                    "north" == l ? (e.top = parseInt(p.css("top")) + p.outerHeight() - d.height(),
                    e.left = parseInt(p.css("left")),
                    e.width = p.outerWidth(),
                    e.height = d.height()) : "south" == l ? (e.top = parseInt(p.css("top")),
                    e.left = parseInt(p.css("left")),
                    e.width = p.outerWidth(),
                    e.height = d.height()) : "east" == l ? (e.top = parseInt(p.css("top")) || 0,
                    e.left = parseInt(p.css("left")) || 0,
                    e.width = d.width(),
                    e.height = p.outerHeight()) : "west" == l && (e.top = parseInt(p.css("top")) || 0,
                    e.left = p.outerWidth() - d.width(),
                    e.width = d.width(),
                    e.height = p.outerHeight()),
                    d.css(e),
            ...
            if (i.panel(a.extend({}, a.fn.layout.paneldefaults, {
                ...
                onResize: function() {
                    ...
                    if (b.length) {
                        ...
                        b.hasClass("layout-expand-title-down") && (c += Math.min(b._outerWidth(), b._outerHeight()),
                        d = 0),
                        b.css({
                            left: c + "px",
                            top: d + "px"
    ...
    a.fn.layout.paneldefaults = a.extend({}, a.fn.panel.defaults, {
        ...
        collapsedContent: function(b) {
            var c = a(this)
              , d = c.panel("options");
            if ("north" == d.region || "south" == d.region)
                return b;
            var e = [];
...
function($) {
    ...
    function createItem(target, div, options) {
        ...
        item.hasClass("menu-sep") || (item.addClass("menu-item"),
        item.empty().append($('<div class="menu-text"></div>').html(itemOpts.text)),
        itemOpts.iconCls && $('<div class="menu-icon"></div>').addClass(itemOpts.iconCls).appendTo(item),
        itemOpts.id && item.attr("id", itemOpts.id),
        itemOpts.onclick && ("string" == typeof itemOpts.onclick ? item.attr("onclick", itemOpts.onclick) : item[0].onclick = eval(itemOpts.onclick)),
        itemOpts.disabled && setDisabled(target, item[0], !0),
        item[0].submenu && $('<div class="menu-rightarrow"></div>').appendTo(item))
    }
    function setMenuSize(a, b) {
        ...
        if (isNaN(parseInt(h)))
            ...
            b.hasClass("menu-top") && c.alignTo) {
                ...
                h > $(window)._outerHeight() && (h = $(window).height());
        b.attr("style", d),
        b.show(),
        b._size($.extend({}, f, {
            width: g,
    ...
    function mouseoverHandler(a) {
        ...
        if (c.length) {
            ...
            c.hasClass("menu-item-disabled"))
                return void c.addClass("menu-active-disabled");
            var d = c[0].submenu;
            d && $(b).menu("show", {
                menu: d,
                parent: c
    ...
    function clickHandler(a) {
        ...
        if (c.length) {
            ...
            if (e.disabled)
                return;
            c[0].submenu || (hideAll(b, d.inline),
    ...
    function hideAll(a, b) {
        var c = $.data(a, "menu");
        return c && $(a).is(":visible") && (hideMenu($(a)),
        b ? $(a).show() : c.options.onHide.call(a)),
        !1
    }
    ...
    function setDisabled(a, b, c) {
        ...
        if (d.hasClass("menu-item")) {
            ...
            e.disabled = c,
            c ? (d.addClass("menu-item-disabled"),
            d[0].onclick = null) : (d.removeClass("menu-item-disabled"),
            d[0].onclick = e.onclick)
        }
    }
    ...
    $(function() {
        $(document).unbind(".menu").bind("mousedown.menu", function(a) {
            ...
            b.length || ($("body>div.menu-top:visible").not(".menu-inline").menu("hide"),
...
function(a) {
    function b(b) {
        ...
        var f = e.attr("name");
        return f && (e.removeAttr("name").attr("switchbuttonName", f),
        d.find(".switchbutton-value").attr("name", f)),
    ...
    function d(b) {
        ...
        d.reversed ? (j.prependTo(h),
        i.insertAfter(k)) : (i.prependTo(h),
        j.insertAfter(k)),
    ...
    function f(b, c) {
        ...
        f.addClass("switchbutton-disabled")) : (e.disabled = !1,
        a(b).add(g).removeAttr("disabled"),
        f.removeClass("switchbutton-disabled"))
    }
    function g(b, c) {
        var d = a.data(b, "switchbutton")
          , e = d.options;
...
function($) {
    ...
    function bindEvents(a) {
        ...
        if ($(a).unbind(".validatebox"),
        !b.novalidate && !b.disabled)
            for (var c in b.events)
                $(a).bind(c + ".validatebox", {
                    target: a
                }, b.events[c])
    }
    function focusEventHandler(a) {
        var b = a.data.target
          ...
          , d = c.options;
        $(b).attr("readonly") || (c.validating = !0,
        c.value = d.val(b),
        function() {
            if ($(b).is(":visible") || (c.validating = !1),
            c.validating) {
                var a = d.val(b);
                c.value != a ? (c.value = a,
                c.vtimer && clearTimeout(c.vtimer),
                c.vtimer = setTimeout(function() {
    ...
    function blurEventHandler(a) {
        ...
        c.vtimer && (clearTimeout(c.vtimer),
        c.vtimer = void 0),
        c.ftimer && (clearTimeout(c.ftimer),
        c.ftimer = void 0),
    ...
    function validate(target) {
        ...
        function _validate() {
            ...
            if (opts.novalidate || opts.disabled)
                return !0;
            if (opts.required && "" == opts.val(target))
                return setTipMessage(opts.missingMessage),
                !1;
            if (opts.validType)
                if ($.isArray(opts.validType)) {
                    for (var a = 0; a < opts.validType.length; a++)
                        if (!doValidate(opts.validType[a]))
                            return !1
    ...
    function setReadonly(a, b) {
        ...
        c.readonly || !c.editable ? ($(a).triggerHandler("blur.validatebox"),
        $(a).addClass("validatebox-readonly").attr("readonly", "readonly")) : $(a).removeClass("validatebox-readonly").removeAttr("readonly")
    }
    $.fn.validatebox = function(a, b) {
        return "string" == typeof a ? $.fn.validatebox.methods[a](this, b) : (a = a || {},
        this.each(function() {
            var b = $.data(this, "validatebox");
            b ? $.extend(b.options, a) : (init(this),
            b = $.data(this, "validatebox", {
...
function(a) {
    function b(b) {
        ...
        var c = a('<span class="textbox"><input class="textbox-text" autocomplete="off"><input type="hidden" class="textbox-value"></span>').insertAfter(b)
          , d = a(b).attr("name");
        return d && (c.find("input.textbox-value").attr("name", d),
    ...
    function c(b) {
        ...
        k.length) {
            ...
            for (var m = 0; m < k.length; m++)
                l.append('<a href="javascript:;" class="textbox-icon ' + k[m].iconCls + '" icon-index="' + m + '" tabindex="-1"></a>')
        }
        if (e.find(".textbox-button").remove(),
        d.buttonText || d.buttonIcon) {
        ...
        d.label ? "object" == typeof d.label ? (c.label = a(d.label),
        c.label.attr("for", g)) : (a(c.label).remove(),
        c.label = a('<label class="textbox-label"></label>').html(d.label),
        c.label.css("textAlign", d.labelAlign).attr("for", g),
        "after" == d.labelPosition ? c.label.insertAfter(e) : c.label.insertBefore(b),
        c.label.removeClass("textbox-label-left textbox-label-right textbox-label-top"),
        c.label.addClass("textbox-label-" + d.labelPosition)) : a(c.label).remove(),
        f(b),
        h(b, d.disabled),
        i(b, d.readonly)
    }
    ...
    function e(b, c) {
        ...
        var f = a.data(b, "textbox")
          ...
          , h = f.textbox
          , i = h.parent();
        if (c && ("object" == typeof c ? a.extend(g, c) : g.width = c),
        isNaN(parseInt(g.width))) {
            var j = a(b).clone();
        ...
        "auto" != g.height && h._size("height", h.outerHeight() - f.label.outerHeight())) : (f.label._size({
            width: g.labelWidth,
            height: h.outerHeight()
        }, h),
        g.multiline || f.label.css("lineHeight", f.label.height() + "px"),
        h._size("width", h.outerWidth() - f.label.outerWidth()))),
    ...
    function f(b) {
        ...
        d.validatebox(a.extend({}, c, {
            ...
            onBeforeValidate: function() {
                ...
                var d = a(this);
                d.is(":focus") || d.val() !== c.value && (c.oldInputValue = d.val(),
                d.val(c.value))
            },
            onValidate: function(d) {
                var e = a(this);
                void 0 != c.oldInputValue && (e.val(c.oldInputValue),
    ...
    function g(b) {
        ...
        if (g.attr("placeholder", d.prompt),
        g.unbind(".textbox"),
        a(c.label).unbind(".textbox"),
        !d.disabled && !d.readonly) {
            c.label && a(c.label).bind("click.textbox", function(c) {
                d.hasFocusMe || (g.focus(),
                a(b).textbox("setSelectionRange", {
                    start: 0,
                    end: g.val().length
                }))
            }),
            g.bind("blur.textbox", function(b) {
                f.hasClass("textbox-focused") && (d.value = a(this).val(),
                ...
                f.addClass("textbox-focused"))
            });
            for (var h in d.inputEvents)
                g.bind(h + ".textbox", {
                    target: b
                }, d.inputEvents[h])
        }
    ...
    function h(b, c) {
        ...
        e.disabled = c,
        e.disabled ? (g.blur(),
        g.validatebox("disable"),
        f.addClass("textbox-disabled"),
        h.attr("disabled", "disabled"),
        a(d.label).addClass("textbox-label-disabled")) : (g.validatebox("enable"),
        f.removeClass("textbox-disabled"),
        h.removeAttr("disabled"),
        a(d.label).removeClass("textbox-label-disabled"))
    }
    function i(b, c) {
        var d = a.data(b, "textbox")
          , e = d.options
    ...
    a.fn.textbox = function(d, f) {
        ...
        this.each(function() {
            ...
            void 0 != d.value && (f.options.originalValue = d.value)) : (f = a.data(this, "textbox", {
        ...
        cloneFrom: function(b, c) {
            return b.each(function() {
                ...
                if (!b.data("textbox")) {
                    ...
                    k.length && (k.attr("for", i),
                    "after" == d.labelPosition ? k.insertAfter(b.next()) : k.insertBefore(b)),
                    a.data(this, "textbox", {
                        options: d,
                        textbox: h,
...
function(a) {
    ...
    function c(b, c, d) {
        ...
        if (f.revealed)
            return void e.textbox("setValue", c);
    ...
    function e(b) {
        ...
        function() {
            if (f.checking) {
                ...
                f.value != a && (f.value = a,
                f.lastTimer && (clearTimeout(f.lastTimer),
                f.lastTimer = void 0),
                c(d, a),
                f.lastTimer = setTimeout(function() {
    ...
    function f(b) {
        ...
        e.checking = !1,
        e.lastTimer && (clearTimeout(e.lastTimer),
        e.lastTimer = void 0),
        d(c)
    }
...
function($) {
    function buildSearchBox(a) {
        ...
        function d(b) {
            ...
            e.searchbox.find("input.textbox-value").attr("name", b.name || b.text),
            $(a).searchbox("resize"))
        }
...
function(a) {
    ...
    function e(b, c) {
        function d(c) {
            ...
            for (var f in c) {
                var i = c[f];
                e(f, i) || g(f, i) || (d.find('input[name="' + f + '"]').val(i),
                d.find('textarea[name="' + f + '"]').val(i),
        ...
        function e(c, d) {
            ...
            return e.length ? (e.switchbutton("uncheck"),
            e.each(function() {
            ...
            !!e.length && (e._propAttr("checked", !1),
            e.each(function() {
                f(a(this).val(), d) && (a(this).data("lable").click(),
                a(this)._propAttr("checked", !0))
            }),
            !0))
        }
        function f(b, c) {
            ...
            if (e.length)
                for (var f = 0; f < h.fieldTypes.length; f++) {
                    var g = h.fieldTypes[f]
                      , i = e.data(g);
                    if (i)
                        return i.options.multiple || i.options.range ? e[g]("setValues", d) : e[g]("setValue", d),
                        !0
                }
            return !1
        }
    ...
    function h(c) {
        ...
        a(c).bind("_change.form", function(b, c) {
            ...
            var c = b.target;
            a(c).hasClass("textbox-text") || (a.inArray(c, d.dirtyFields) == -1 && d.dirtyFields.push(c),
            d.onChange.call(this, c))
        }),
        k(c, d.novalidate)
    }
    ...
    a.fn.numberbox.defaults = a.extend({}, a.fn.textbox.defaults, {
        ...
        filter: function(b) {
            ...
            if (b.metaKey || b.ctrlKey)
                return !0;
            if (a.inArray(String(b.which), ["46", "8", "13", "0"]) >= 0)
                return !0;
            var e = a("<span></span>");
        ...
        parser: function(b) {
            ...
            parseFloat(b) != c.value && (c.groupSeparator && (b = a.trim(b.replace(new RegExp("\\" + c.groupSeparator,"g"), ""))),
...
function(a) {
    ...
    function d(b) {
        ...
        a(b).unbind(".calendar").bind("mouseover.calendar", function(a) {
            ...
            var k = c(a.target);
            if (k.hasClass("calendar-menu-next") || k.hasClass("calendar-nextyear"))
                f(1);
            else if (k.hasClass("calendar-menu-prev") || k.hasClass("calendar-prevyear"))
                f(-1);
            else if (k.hasClass("calendar-menu-month"))
                j.find(".calendar-selected").removeClass("calendar-selected"),
                k.addClass("calendar-selected"),
                d(!0);
            else if (k.hasClass("calendar-prevmonth"))
                h(-1);
            else if (k.hasClass("calendar-nextmonth"))
                h(1);
            else if (k.hasClass("calendar-text"))
                j.is(":visible") ? j.hide() : e(b);
            else if (k.hasClass("calendar-day")) {
                if (k.hasClass("calendar-disabled"))
                    return;
                var l = i.current;
    ...
    function e(b) {
        ...
        a(b).find(".calendar-menu-month-inner").is(":empty")) {
            ...
            for (var d = a('<table class="calendar-mtable"></table>').appendTo(a(b).find(".calendar-menu-month-inner")), e = 0, f = 0; f < 3; f++)
                for (var g = a("<tr></tr>").appendTo(d), h = 0; h < 4; h++)
                    a('<td class="calendar-nav calendar-menu-month"></td>').html(c.months[e++]).attr("abbr", e).appendTo(g)
        }
    ...
    function f(b, c, d) {
        for (var e = a.data(b, "calendar").options, f = [], g = new Date(c,d,0).getDate(), h = 1; h <= g; h++)
            f.push([c, d, h]);
    ...
    function g(b) {
        ...
        for (var m = f(b, c.year, c.month), l = 0; l < m.length; l++) {
            var n = m[l]
              , o = "";
            if (0 == l ? o = "calendar-first" : l == m.length - 1 && (o = "calendar-last"),
            k.push('<tr class="' + o + '">'),
            c.showWeek) {
            ...
            for (var q = 0; q < n.length; q++) {
                var r = n[q]
                  ...
                  , w = ""
                  , x = "";
                "string" == typeof v ? x = v : v && (w = v.class || "",
                x = v.style || "");
                var o = "calendar-day";
...
function(a) {
    function b(b) {
        ...
        if ("left" == e.spinAlign || "right" == e.spinAlign) {
            ...
            e.spinArrow = !1,
            "vertical" == e.spinAlign ? ("top" != e.buttonAlign && (e.buttonAlign = "bottom"),
            e.clsLeft = "textbox-button-bottom",
            e.clsRight = "textbox-button-top") : (e.clsLeft = "textbox-button-left",
            e.clsRight = "textbox-button-right");
        ...
        a(b).attr("spinnerName", a(b).attr("textboxName")),
        d.spinner = a(b).next(),
        d.spinner.addClass("spinner"),
        e.spinArrow) {
...
function($) {
        ...
        })),
        {
            ...
            remove: function(a) {
                ...
                for (var c in h.cache)
                    c.indexOf(a) == -1 && b.push([c, h.cache[c].width]);
                h.cache = {},
                this.add(b)
            },
            dirty: function(a) {
            ...
            return $(target).children("thead").each(function() {
                ...
                $(this).find("tr").each(function() {
                    ...
                    $(this).find("th").each(function() {
                        ...
                        if (col.width && String(col.width).indexOf("%") == -1 && (col.width = parseInt(col.width)),
                        th.attr("editor")) {
    ...
    function buildGrid(target) {
        function createColumnHeader(a, b, c) {
            if (b) {
                ...
                var f = []
                  , g = []
                  , h = [];
                opts.sortName && (f = opts.sortName.split(","),
                g = opts.sortOrder.split(","));
                for (var i = $('<table class="datagrid-htable" border="0" cellspacing="0" cellpadding="0"><tbody></tbody></table>').appendTo(a), j = 0; j < b.length; j++)
                    for (var k = $('<tr class="datagrid-header-row"></tr>').appendTo($("tbody", i)), l = b[j], m = 0; m < l.length; m++) {
                        ...
                        n.colspan && (o += 'colspan="' + n.colspan + '" ',
                        n.id || (n.id = ["datagrid-td-group" + DATAGRID_SERNO, j, m].join("-"))),
                        n.id && (o += 'id="' + n.id + '"');
                        ...
                        if (n.checkbox)
                            ...
                            if (r >= 0 && q.addClass("datagrid-sort-" + g[r]),
                            n.sortable && q.addClass("datagrid-sort"),
                            0 == n.resizable && q.attr("resizable", "false"),
                            n.width) {
                            ...
                            $('<div class="datagrid-cell-group"></div>').html(n.title).appendTo(p);
                        n.hidden && (p.hide(),
                        h.push(n.field))
                    }
        ...
        var state = $.data(target, "datagrid")
          ...
          , dc = state.dc
          , panel = state.panel;
        if (state.ss = $(target).datagrid("createStyleSheet"),
        panel.panel($.extend({}, opts, {
            id: null,
            doSize: !1,
            onResize: function(a, b) {
                $.data(target, "datagrid") && (setBodySize(target),
        ...
        createColumnHeader(dc.header1, opts.frozenColumns, !0),
        createColumnHeader(dc.header2, opts.columns, !1),
        createColumnStyle(),
        ...
        opts.toolbar)
            ...
            $("div.datagrid-toolbar", panel).remove();
        if ($("div.datagrid-pager", panel).remove(),
        opts.pagination) {
    ...
    function bindEvents(a) {
        ...
        f.unbind(".datagrid");
        for (var g in d.headerEvents)
            f.bind(g + ".datagrid", d.headerEvents[g]);
    ...
    function headerClickEventHandler(a) {
        ...
        if (d.length) {
            if (c.singleSelect && c.selectOnCheck)
                return !1;
    ...
    function clickEventHandler(a) {
        ...
        if (b) {
            var c = getTableTarget(b)
              ...
              , e = getTrIndex(b)
              , f = $(a.target);
            if (f.parent().hasClass("datagrid-cell-check"))
                d.singleSelect && d.selectOnCheck ? (f._propAttr("checked", !f.is(":checked")),
                checkRow(c, e)) : f.is(":checked") ? (f._propAttr("checked", !1),
                checkRow(c, e)) : (f._propAttr("checked", !0),
                uncheckRow(c, e));
            else {
                ...
                if (1 == d.singleSelect)
                    ...
                    if (a.metaKey || a.ctrlKey)
                        b.hasClass("datagrid-row-selected") ? unselectRow(c, e) : selectRow(c, e);
                    else if (a.shiftKey) {
                        $(c).datagrid("clearSelections");
                        for (var j = Math.min(d.lastSelectedIndex || 0, e), k = Math.max(d.lastSelectedIndex || 0, e), l = j; l <= k; l++)
                            selectRow(c, l)
                    } else
                        $(c).datagrid("clearSelections"),
                        selectRow(c, e),
        ...
        function b() {
            if (g.fitColumns) {
                ...
                if (b) {
                    ...
                    for (var h = 0; h < c.length; h++) {
                        var n = c[h];
                        n.col.boxWidth + n.addingWidth > 0 && (n.col.boxWidth += n.addingWidth,
                        n.col.width += n.addingWidth)
                    }
                    f.leftWidth = l,
    ...
    function fixColumnSize(a, b) {
        ...
        b)
            ...
            for (var g = getColumnFields(a, !0).concat(getColumnFields(a, !1)), h = 0; h < g.length; h++)
                c(g[h]);
        f.css("table-layout", ""),
        fixMergedSize(a),
        fixRowHeight(a),
    ...
    function fixMergedSize(a, b) {
        ...
        b.each(function() {
            var b = $(this)
              , c = b.attr("colspan") || 1;
            if (c > 1) {
                for (var d = getColumnOption(a, b.attr("field")), e = d.boxWidth + d.deltaWidth - 1, f = 1; f < c; f++)
                    b = b.next(),
    ...
    function getColumnLayout(a, b) {
        ...
        for (var e = $.data(a, "datagrid").options, f = b ? e.frozenColumns : e.columns, g = [], h = c(), i = 0; i < f.length; i++)
            g[i] = new Array(h);
    ...
    function selectRow(a, b, c, d) {
        ...
        g && 0 != f.onBeforeSelect.apply(a, getArguments(a, [b, g])) && (f.singleSelect && (unselectAll(a, !0),
        e.selectedRows = []),
        !c && f.checkOnSelect && checkRow(a, b, !0),
        f.idField && addArrayItem(e.selectedRows, f.idField, g),
        f.finder.getTr(a, b).addClass("datagrid-row-selected"),
        f.onSelect.apply(a, getArguments(a, [b, g])),
        !d && f.scrollOnSelect && scrollTo(a, b))
    }
    function unselectRow(a, b, c) {
        ...
        f && 0 != e.onBeforeUnselect.apply(a, getArguments(a, [b, f])) && (!c && e.checkOnSelect && uncheckRow(a, b, !0),
        e.finder.getTr(a, b).removeClass("datagrid-row-selected"),
        e.idField && removeArrayItem(d.selectedRows, e.idField, f[e.idField]),
    ...
    function selectAll(a, b) {
        ...
        if (!b && d.checkOnSelect && checkAll(a, !0),
        d.finder.getTr(a, "", "allbody").addClass("datagrid-row-selected"),
        d.idField)
            for (var g = 0; g < e.length; g++)
                addArrayItem(f, d.idField, e[g]);
    ...
    function unselectAll(a, b) {
        var c = $.data(a, "datagrid")
          ...
          , e = d.finder.getRows(a)
          , f = $.data(a, "datagrid").selectedRows;
        if (!b && d.checkOnSelect && uncheckAll(a, !0),
        d.finder.getTr(a, "", "selected").removeClass("datagrid-row-selected"),
        d.idField)
            for (var g = 0; g < e.length; g++)
                removeArrayItem(f, d.idField, e[g][d.idField]);
    ...
    function checkRow(a, b, c) {
        ...
        if (f && 0 != e.onBeforeCheck.apply(a, getArguments(a, [b, f]))) {
            e.singleSelect && e.selectOnCheck && (uncheckAll(a, !0),
            d.checkedRows = []),
            ...
            g = e.finder.getTr(a, "", "checked", 2),
            g.length == e.finder.getRows(a).length) {
                var h = d.dc;
    ...
    function checkAll(a, b) {
        ...
        d.idField)
            for (var i = 0; i < e.length; i++)
                addArrayItem(c.checkedRows, d.idField, e[i]);
    ...
    function uncheckAll(a, b) {
        ...
        d.idField)
            for (var i = 0; i < e.length; i++)
                removeArrayItem(c.checkedRows, d.idField, e[i][d.idField]);
    ...
    function beginEdit(a, b) {
        ...
        validateRow(a, b),
        c.onBeginEdit.apply(a, getArguments(a, [b, e])))
    }
    ...
    function updateRow(a, b) {
        ...
        f && (indexOfArray(c.insertedRows, e) == -1 && indexOfArray(c.updatedRows, e) == -1 && c.updatedRows.push(e),
        d.view.updateRow.call(d.view, a, b.index, b.row))
    }
    ...
    function acceptChanges(a) {
        for (var b = $.data(a, "datagrid").data, c = !0, d = 0, e = b.rows.length; d < e; d++)
            validateRow(a, d) ? $(a).datagrid("endEdit", d) : c = !1;
        c && initChanges(a)
    }
    ...
    function request(a, b, c) {
        ...
        0 == d.onBeforeLoad.call(a, e))
            return void d.view.setEmptyMsg(a);
        $(a).datagrid("loading");
    ...
    function mergeCells(a, b) {
        ...
        1 != b.rowspan || 1 != b.colspan) {
            ...
            if (e.length) {
                ...
                for (var g = 1; g < b.rowspan && (e = e.next(),
                e.length); g++)
                    c(e.find('td[field="' + b.field + '"]'), b.colspan);
                fixMergedSize(a, f)
            }
        }
    }
    ...
    $.fn.datagrid.methods = {
        ...
        showColumn: function(a, b) {
            return a.each(function() {
                ...
                a.hidden && (a.hidden = !1,
                $(this).datagrid("getPanel").find('td[field="' + b + '"]').show(),
                fixColumnSpan(this, b, 1),
        ...
        hideColumn: function(a, b) {
            return a.each(function() {
                var a = $(this).datagrid("getColumnOption", b);
                a.hidden || (a.hidden = !0,
                $(this).datagrid("getPanel").find('td[field="' + b + '"]').hide(),
                fixColumnSpan(this, b, -1),
                $(this).datagrid("fitColumns"))
            })
        },
    ...
    var defaultView = {
        ...
        renderRow: function(a, b, c, d, e) {
            ...
            for (var i = 0; i < b.length; i++) {
                ...
                if (k) {
                    ...
                    var p = "";
                    k.checkbox || (k.align && (p += "text-align:" + k.align + ";"),
                    f.nowrap ? f.autoRowHeight && (p += "height:auto;") : p += "white-space:normal;height:auto;"),
                    g.push('<div style="' + p + '" '),
                    ...
                    g.push(' name="' + j + '" value="' + (void 0 != l ? l : "") + '">')) : k.formatter ? g.push(k.formatter(l, e, d)) : g.push(l),
                    g.push("</div>"),
                    g.push("</td>")
                }
            }
        ...
        getStyleValue: function(a) {
            var b = ""
              , c = "";
            return "string" == typeof a ? c = a : a && (b = a.class || "",
            c = a.style || ""),
            {
                c: b,
                s: c
        ...
        setEmptyMsg: function(a) {
            ...
            if (d && this.renderEmptyRow(a),
            c.emptyMsg && (b.dc.view.children(".datagrid-empty").remove(),
            d)) {
...
function(a) {
    ...
    var e = a.extend({}, a.fn.datagrid.defaults.view, {
        ...
        renderGroup: function(b, c, d, e) {
            ...
            (e && (g.rownumbers || g.frozenColumns.length) || !e && !g.rownumbers && !g.frozenColumns.length) && (i.push('<span class="datagrid-group-expander">'),
            i.push('<span class="datagrid-row-expander datagrid-row-collapse">&nbsp;</span>'),
            i.push("</span>")),
    ...
    a.extend(e, {
        ...
        insertRow: function(b, c, d) {
            ...
            if (!g.data.rows.length)
                return void a(b).datagrid("loadData", [d]);
            ...
            j ? (void 0 != c && null != c || (c = g.data.rows.length),
            c < j.startIndex ? c = j.startIndex : c > j.startIndex + j.rows.length && (c = j.startIndex + j.rows.length),
            a.fn.datagrid.defaults.view.insertRow.call(this, b, c, d),
            c >= j.startIndex + j.rows.length && (e(c, !0),
            e(c, !1)),
            j.rows.splice(c - j.startIndex, 0, d)) : (j = {
                value: d[h.groupField],
                rows: [d],
                startIndex: g.data.rows.length
            },
            f = this.groups.length,
            i.body1.append(this.renderGroup.call(this, b, f, j, !0)),
            i.body2.append(this.renderGroup.call(this, b, f, j, !1)),
            this.groups.push(j),
            g.data.rows.push(d)),
        ...
        deleteRow: function(b, c) {
            ...
            var j = this.groups[i];
            if (j.rows.length > 1)
                j.rows.splice(c - j.startIndex, 1),
                this.refreshGroupTitle(b, i);
            else {
                g.children("div.datagrid-group[group-index=" + i + "]").remove();
                for (var k = i + 1; k < this.groups.length; k++)
                    g.children("div.datagrid-group[group-index=" + k + "]").attr("group-index", k - 1);
...
function(a) {
    function b(b) {
        ...
        a(b).datagrid(a.extend({}, e, {
            ...
            onBeforeSortColumn: function(a, c) {
                if (0 == e.onBeforeSortColumn.call(b, a, c))
                    return !1
            },
    ...
    function c(b, c) {
        ...
        if (!f.body1.is(":empty") && (!e.nowrap || e.autoRowHeight) && void 0 != c)
            for (var g = t(b, c), h = 0; h < g.length; h++)
                d(g[h][e.idField]);
    ...
    function f(b) {
        ...
        if (d.length && d.parent().length) {
            ...
            if (c.hasClass("tree-hit"))
                y(f, e);
            else if (c.hasClass("tree-checkbox"))
                h(f, e);
            else {
                var i = a(f).datagrid("options");
                if (c.parent().hasClass("datagrid-cell-check") || i.singleSelect || !b.shiftKey)
                    ...
                    for (var r = m; r <= n; r++)
                        a(f).treegrid("selectRow", j[r][i.idField]);
    ...
    function h(b, c, d, e) {
        ...
        if (g.checkbox) {
            ...
            if (h.checkState) {
                ...
                h._checked = d,
                d) {
                    if (m.hasClass("tree-checkbox1"))
                        return
                } else if (m.hasClass("tree-checkbox0"))
                    return;
                (e || 0 != g.onBeforeCheckNode.call(b, h, d)) && (g.cascadeCheck ? (j(b, h, d),
                k(b, h)) : i(b, h, d ? "1" : "0"),
                e || g.onCheckNode.call(b, h, d))
            }
        }
    }
    function i(b, c, d) {
        ...
        if (c.checkState && void 0 != d) {
            ...
            i.length && (c.checkState = ["unchecked", "checked", "indeterminate"][d],
            c.checked = "checked" == c.checkState,
            i.removeClass("tree-checkbox0 tree-checkbox1 tree-checkbox2"),
            i.addClass("tree-checkbox" + d),
            0 == d ? a.easyui.removeArrayItem(f, g.idField, c[g.idField]) : a.easyui.addArrayItem(f, g.idField, c))
        }
    }
    function j(b, c, d) {
        var e = d ? 1 : 0;
    ...
    function l(b) {
        ...
        if (a.easyui.forEach(b.children || [], !1, function(a) {
            a.checkState && (c++,
            "checked" == a.checkState ? e++ : "unchecked" == a.checkState && d++)
        }),
        0 != c) {
            var f = 0;
    ...
    function m(b, c) {
        ...
        if (d.checkbox) {
            ...
            if (d.view.hasCheckbox(b, e))
                if (g.length || (e.checkState = e.checkState || "unchecked",
                a('<span class="tree-checkbox"></span>').insertBefore(f.find(".tree-title"))),
                "checked" == e.checkState)
                    h(b, c, !0, !0);
                else if ("unchecked" == e.checkState)
                    h(b, c, !1, !0);
                else {
                    var i = l(e);
    ...
    function o(b, e, f, g, h) {
        ...
        if (g || (o.empty(),
        p.empty()),
        j.view.onBeforeRender && j.view.onBeforeRender.call(j.view, b, e, f),
        j.view.render.call(j.view, b, o, !0),
        j.view.render.call(j.view, b, p, !1),
        j.showFooter && (j.view.renderFooter.call(j.view, b, k.footer1, !0),
        j.view.renderFooter.call(j.view, b, k.footer2, !1)),
        j.view.onAfterRender && j.view.onAfterRender.call(j.view, b),
        !e && j.pagination) {
    ...
    function p(b, c, d, e, f) {
        ...
        if (0 != g.onBeforeLoad.call(b, j, i)) {
            ...
            0 == l && (k.removeClass("tree-loading"),
            a(b).treegrid("loaded"))
        }
    }
    function q(a) {
    ...
    function F(b) {
        ...
        if (!g.lines)
            return void f.treegrid("getPanel").removeClass("tree-lines");
    ...
    a.fn.treegrid.methods = {
        ...
        update: function(b, c) {
            return b.each(function() {
                ...
                void 0 != d.checked && (d = v(this, c.id),
                a.extend(d, {
        ...
        setSelectionState: function(b) {
            return b.each(function() {
                ...
                for (var b = a(this).data("treegrid"), c = 0; c < b.tmpIds.length; c++)
                    h(this, b.tmpIds[c], !0, !0);
                b.tmpIds = []
            })
        },
    ...
    var G = a.extend({}, a.fn.datagrid.defaults.view, {
        render: function(b, c, d) {
            function e(c, d, j) {
                for (var k = a(b).treegrid("getParent", j[0][f.idField]), l = (k ? k.children.length : a(b).treegrid("getRoots").length) - j.length, m = ['<table class="datagrid-btable" cellspacing="0" cellpadding="0" border="0"><tbody>'], n = 0; n < j.length; n++) {
                    ...
                    if (m.push('<tr id="' + t + '" node-id="' + o[f.idField] + '" ' + r + " " + s + ">"),
                    m = m.concat(i.renderRow.call(i, b, g, c, d, o)),
                    m.push("</tr>"),
        ...
        renderRow: function(b, c, d, e, f) {
            ...
            for (var j = 0; j < c.length; j++) {
                ...
                if (l) {
                    ...
                    var p = "";
                    if (l.checkbox || (l.align && (p += "text-align:" + l.align + ";"),
                    h.nowrap ? h.autoRowHeight && (p += "height:auto;") : p += "white-space:normal;height:auto;"),
                    i.push('<div style="' + p + '" '),
                    l.checkbox ? i.push('class="datagrid-cell-check ') : i.push('class="datagrid-cell ' + l.cellClass),
                    i.push('">'),
                    l.checkbox)
                        f.checked ? i.push('<input type="checkbox" checked="checked"') : i.push('<input type="checkbox"'),
                        i.push(' name="' + k + '" value="' + (void 0 != f[k] ? f[k] : "") + '">');
                    else {
                        var q = null;
                        if (q = l.formatter ? l.formatter(f[k], f) : f[k],
                        k == h.treeField) {
                            for (var r = 0; r < e; r++)
                                i.push('<span class="tree-indent"></span>');
                            if ("closed" == f.state ? (i.push('<span class="tree-hit tree-collapsed"></span>'),
                            i.push('<span class="tree-icon tree-folder ' + (f.iconCls ? f.iconCls : "") + '"></span>')) : f.children && f.children.length ? (i.push('<span class="tree-hit tree-expanded"></span>'),
                            i.push('<span class="tree-icon tree-folder tree-folder-open ' + (f.iconCls ? f.iconCls : "") + '"></span>')) : (i.push('<span class="tree-indent"></span>'),
                            i.push('<span class="tree-icon tree-file ' + (f.iconCls ? f.iconCls : "") + '"></span>')),
                            this.hasCheckbox(b, f)) {
                                var s = 0
                                  , t = a.easyui.getArrayItem(g.checkedRows, h.idField, f[h.idField]);
                                if (t)
                                    s = "checked" == t.checkState ? 1 : 2,
                                    ...
                                    u && "checked" == u.checkState && h.cascadeCheck ? (s = 1,
                                    f.checked = !0,
                                    a.easyui.addArrayItem(g.checkedRows, h.idField, f)) : f.checked && a.easyui.addArrayItem(g.tmpIds, f[h.idField]),
        ...
        hasCheckbox: function(b, c) {
            var d = a.data(b, "treegrid").options;
            if (d.checkbox) {
                if (a.isFunction(d.checkbox))
                    return !!d.checkbox.call(b, c);
                if (!d.onlyLeafCheck)
                    return !0;
                if (!("open" != c.state || c.children && c.children.length))
                    return !0
        ...
        transfer: function(b, c, d) {
            ...
            for (var f = a.data(b, "treegrid").options, g = a.extend([], d), h = e(c, g), i = a.extend([], h); i.length; ) {
                var j = i.shift()
                  , k = e(j[f.idField], g);
                k.length && (j.children ? j.children = j.children.concat(k) : j.children = k,
                i = i.concat(k))
            }
            return h
        }
    ...
    var c = a.extend({}, a.fn.datagrid.defaults.view, {
        render: function(b, c, d) {
            ...
            if (f.groupField) {
                ...
                e.data.rows = g.rows;
                for (var h = [], i = 0; i < g.groups.length; i++)
                    h.push(this.renderGroup.call(this, b, i, g.groups[i], d));
                a(c).html(h.join(""))
            } else
                a(c).html(this.renderTable(b, 0, e.data.rows, d))
        },
        renderGroup: function(b, c, d, e) {
            var f = a.data(b, "datagrid")
              , g = f.options
            ...
            e || (h.push('<span class="datagrid-group-title">'),
            h.push(g.groupFormatter.call(b, d.value, d.rows)),
            h.push("</span>")),
            h.push("</div>"),
...
function(a) {
    ...
    function g(b) {
        var c = b.data.target
          ...
          , e = d.data("combo")
          , f = d.combo("options");
        switch (e.panel.panel("options").comboTarget = c,
        b.keyCode) {
        case 38:
        ...
        case 27:
            ...
            break;
        default:
            f.editable && (e.timer && clearTimeout(e.timer),
            e.timer = setTimeout(function() {
                var a = d.combo("getText");
                e.previousText != a && (e.previousText = a,
                d.combo("showPanel"),
                f.keyHandler.query.call(c, a, b),
                d.combo("validate"))
            }, f.delay))
        }
    }
    ...
    function l(b, c) {
        ...
        var h = k(b);
        if (f.find(".textbox-value").remove(),
        c.length)
            if (g.multivalue)
                for (var i = 0; i < c.length; i++)
                    d(c[i]);
            else
                d(c.join(g.separator));
        var j = function() {
        ...
        j && (a(b).val(c.join(g.separator)),
        g.multiple ? g.onChange.call(b, c, h) : g.onChange.call(b, c[0], h[0]),
        a(b).closest("form").trigger("_change", [b]))
    }
    function m(a) {
        var b = k(a);
        return b[0]
    ...
    a.fn.combo = function(c, d) {
        ...
        this.each(function() {
            ...
            d ? (a.extend(d.options, c),
            void 0 != c.value && (d.options.originalValue = c.value)) : (d = a.data(this, "combo", {
...
function(a) {
    ...
    function d(b, c) {
        ...
        if (g.length ? "next" == c ? (g = g.nextAll(h),
        g.length || (g = f.children(h))) : (g = g.prevAll(h),
        g.length || (g = f.children(i))) : g = f.children("next" == c ? h : i),
        g.length) {
            g.addClass("combobox-item-hover");
            var j = d.finder.getRow(b, g);
            j && (a(b).combobox("scrollTo", j[d.valueField]),
            d.selectOnNavigation && e(b, j[d.valueField]))
        }
    }
    function e(b, c, d) {
        var e = a.data(b, "combobox").options
          , f = a(b).combo("getValues");
        a.inArray(c + "", f) == -1 && (e.multiple ? f.push(c) : f = [c],
        g(b, f, d))
    }
    function f(b, c) {
        var d = (a.data(b, "combobox").options,
    ...
    function g(b, c, d) {
        ...
        a.map(g, function(d) {
            if (a.easyui.indexOfArray(c, d) == -1) {
                ...
                f.hasClass("combobox-item-selected") && (f.removeClass("combobox-item-selected"),
                e.onUnselect.call(b, e.finder.getRow(b, d)))
            }
        });
        for (var h = null, i = [], j = [], k = 0; k < c.length; k++) {
            ...
            if (n) {
                ...
                o.hasClass("combobox-item-selected") || (o.addClass("combobox-item-selected"),
                e.onSelect.call(b, n))
            }
            i.push(l),
            j.push(m)
        }
        if (d || a(b).combo("setText", j.join(e.separator)),
        e.showItemIcon) {
            var p = a(b).combobox("textbox");
            p.removeClass("textbox-bgicon " + e.textboxIconCls),
            h && h.iconCls && (p.addClass("textbox-bgicon " + h.iconCls),
            e.textboxIconCls = h.iconCls)
        }
            ...
            a.map(k, function(c) {
                ...
                for (var g = 0; g < m.length; g++) {
                    ...
                    if (h.filter.call(b, c, i)) {
                        ...
                        l.toLowerCase() == c.toLowerCase() && (d = k,
                        h.reversed ? j = p : e(b, k, !0)),
                        h.groupField && f != o && (h.finder.getGroupEl(b, o).show(),
                        f = o)
                    }
                }
                n.push(d)
    ...
    function o(b) {
        ...
        if (c) {
            ...
            if (g.length && !g.hasClass("combobox-item-disabled")) {
                ...
                if (h) {
                    d.blurTimer && (clearTimeout(d.blurTimer),
                    d.blurTimer = null),
                    d.onClick.call(c, h);
                    var i = h[d.textField];
        ...
        if (c) {
            ...
            if ("sticky" == d.groupPosition) {
                ...
                a(this).children(".combobox-group:visible").each(function() {
                    ...
                    if (b.position().top < 0 && i.position().top > 0)
                        return e.show().html(b.html()),
                        !1
                })
            }
        }
      ...
      , r = {
        render: function(b, c, d) {
            ...
            for (var g = [], h = void 0, i = 0; i < d.length; i++) {
                ...
                g.push(f.groupFormatter ? f.groupFormatter.call(b, l) : l),
                g.push("</div>")) : e.groups[e.groups.length - 1].count++ : h = void 0;
    ...
    a.fn.combobox.defaults = a.extend({}, a.fn.combo.defaults, {
        ...
        inputEvents: a.extend({}, a.fn.combo.defaults.inputEvents, {
            blur: function(b) {
                ...
                (d.reversed || d.limitToList) && (d.blurTimer && clearTimeout(d.blurTimer),
                d.blurTimer = setTimeout(function() {
                    var b = a(c).parent().length;
                    if (b) {
                        if (d.reversed)
                            a(c).combobox("setValues", a(c).combobox("getValues"));
        ...
        c.unbind(".tagbox").bind("click.tagbox", function(c) {
            ...
            if (!d.disabled && !d.readonly) {
                if (a(c.target).hasClass("tagbox-remove")) {
                    ...
                    if (0 == d.onBeforeRemoveTag.call(b, f[e]))
                        return;
...
function(a) {
    function b(b) {
        ...
        function f(b) {
            ...
            g.calendar.appendTo(d),
            g.calendar[0].target = b,
            "auto" != h.panelHeight) {
                var e = c.height();
        ...
        !g.calendar) {
            ...
            if (h.sharedCalendar) {
                ...
                g.calendar = a("<div></div>").appendTo(j).calendar();
    ...
    function e(b, c, d) {
        ...
        d ? a(b).combo("setValue", c) : (c && (c = f.formatter.call(b, g.calendar("options").current)),
        a(b).combo("setText", c).combo("setValue", c))
    }
...
function($) {
    ...
    function buildSlider(a) {
        function b(b, c) {
            var e = pos2value(a, b)
              , f = Math.abs(e % d.step);
            if (f < d.step / 2 ? e -= f : e = e - f + d.step,
            d.range) {
                var g = d.value[0]
                  , h = d.value[1]
                  , i = parseFloat((g + h) / 2);
    ...
    function setValues(a, b) {
        ...
        for (var h = $(a).attr("sliderName") || "", i = 0; i < b.length; i++) {
            ...
            var l = e.find(".slider-handle:eq(" + i + ")")
              ...
              , n = value2pos(a, j);
            if (d.showTip ? (m.show(),
            m.html(d.tipFormatter.call(a, j))) : m.hide(),
            "h" == d.mode) {
                var o = "left:" + n + "px;";
                l.attr("style", o),
...
function(a) {
    ...
    function c(b, c) {
        ...
        i.each(function() {
            ...
            g.width() > c.boxWidth + c.deltaWidth - 1 && (c.boxWidth = g.width() - c.deltaWidth + 1,
            c.width = c.boxWidth + c.deltaWidth,
            e = !0)
        }),
    ...
    function i(c) {
        ...
        a(c)[d]("options").pageNumber = 1,
        a(c)[d]("loadData", e.filterSource || e.data))
    }
    function j(b, c, d) {
        ...
        if (!h.remoteFilter) {
            if (g.filterSource) {
                if (h.isSorting)
                    ...
                    g.filterSource = c;
                else if (g.filterSource.total += c.length,
                g.filterSource.rows = g.filterSource.rows.concat(c.rows),
                d)
                    return h.filterMatcher.call(e, c)
            } else
                g.filterSource = c;
    ...
    function l(d, e) {
        ...
        function h(b) {
            ...
            for (var i = 0; i < f.length; i++) {
                ...
                if (p && p.hidden && q.hide(),
                "_" != k && (!p || !p.checkbox && !p.expander)) {
                    var r = l(k);
    ...
    a.extend(a.fn.datagrid.methods, {
        ...
        appendRow: function(b, c) {
            ...
            return b.each(function() {
                ...
                b.filterSource && (b.filterSource.total++,
                b.filterSource.rows != b.data.rows && b.filterSource.rows.push(c))
            }),
            d
        },
        deleteRow: function(b, c) {
            return b.each(function() {
                ...
                if (b.filterSource && d.idField)
                    if (b.filterSource.rows == b.data.rows)
        ...
        defaultFilterOptions: {
            onInit: function(c) {
                function d() {
                    ...
                    a(c)[e]("doFilter")) : b && (a(c)[e]("removeFilterRule", g),
                    a(c)[e]("doFilter"))
                }
                var e = b(c)
                  , f = a(c)[e]("options")
    ...
    a.extend(a.fn.datagrid.methods, {
        ...
        disableFilter: function(c) {
            return c.each(function() {
                ...
                for (var h in e.filterCache)
                    a(e.filterCache[h]).appendTo(g);
                var i = d.data;
                d.filterSource && (i = d.filterSource,
                a.map(i.rows, function(a) {
                    a.children = void 0
                })),
                a(this)[c]({
                    data: i,
...
function(a) {
    ...
    function f(b) {
        ...
        a(b).datagrid(a.extend({}, c, {
            onDblClickCell: function(d, e, f) {
                c.editing && (a(this).edatagrid("editRow", d),
                g(b, e)),
                c.onDblClickCell && c.onDblClickCell.call(b, d, e, f)
            },
            onClickCell: function(d, e, f) {
                c.editing && c.editIndex >= 0 && (a(this).edatagrid("editRow", d),
                g(b, e)),
                c.onClickCell && c.onClickCell.call(b, d, e, f)
            },
            onBeforeEdit: function(e, f) {
                return (!c.onBeforeEdit || 0 != c.onBeforeEdit.call(b, e, f)) && (c.autoSave && d(this),
                void (c.originalRow = a.extend(!0, [], f)))
            },
            onAfterEdit: function(d, f) {
                ...
                if (g) {
                    ...
                    h ? a.post(g, f, function(e) {
                        ...
                        if (e.isNewRecord = null,
                        a(b).datagrid("updateRow", {
                            index: d,
                            row: e
                        }),
                        c.tree) {
            ...
            onBeforeLoad: function(d) {
                if (0 == c.onBeforeLoad.call(b, d))
                    return !1;
                if (a(this).edatagrid("cancelRow"),
                c.tree) {
    ...
    function g(b, c) {
        var d, e = a(b).edatagrid("options"), f = a(b).datagrid("getEditor", {
            index: e.editIndex,
            field: c
        });
        if (f)
    ...
    a.fn.edatagrid.methods = {
            ...
            return b.each(function() {
                ...
                if (e != c)
                    if (b.datagrid("validateRow", e)) {
                        if (e >= 0 && 0 == d.onBeforeSave.call(this, e))
                            return void setTimeout(function() {
                                b.datagrid("selectRow", e)
                            }, 0);
                        if (b.datagrid("endEdit", e),
                        b.datagrid("beginEdit", c),
                        !b.edatagrid("isEditing", c))
                            return;
                        d.editIndex = c,
                        g(this);
        ...
        addRow: function(b, c) {
            return b.each(function() {
                function b(a, b) {
                    void 0 == a ? (d.datagrid("appendRow", b),
                    e.editIndex = f.length - 1) : (d.datagrid("insertRow", {
                        index: a,
                        row: b
                    }),
                    e.editIndex = a)
                ...
                if (e.editIndex >= 0) {
                    if (!d.datagrid("validateRow", e.editIndex))
                        return void d.datagrid("selectRow", e.editIndex);
                    if (0 == e.onBeforeSave.call(this, e.editIndex))
                        return void setTimeout(function() {
        ...
        saveRow: function(b) {
            return b.each(function() {
                ...
                if (c.editIndex >= 0) {
                    if (0 == c.onBeforeSave.call(this, c.editIndex))
                        return void setTimeout(function() {
                        ...
                        if (b.isNewRecord)
                            ...
                            a.post(e.destroyUrl, {
                                ...
                                if (e.tree) {
                                    ...
                                    d.datagrid("cancelEdit", g),
                                    d.datagrid("deleteRow", g);
                                e.onDestroy.call(d[0], g, b);
                                var j = d.datagrid("getPager");
                                j.length && !d.datagrid("getRows").length && (d.datagrid("options").pageNumber = j.pagination("options").pageNumber,
                                d.datagrid("reload"))
                            }, "json")
                        } else
                            d.datagrid("cancelEdit", c),
        ...
        onChange: function(b, c) {
            ...
            if (f.length > 0)
                for (var h = 0; h < f.length; h++)
                    g = d.initUrl.replace(f[h], encodeURI(encodeURI(b)));
            e.iCombobox("reload", g)
        },
        onSelect: function(b) {
            var c = a(this).iCombobox("options");
            a("#" + c.id);
            if (a(this).iCombobox("hidePanel"),
            c.param) {
...
function(a) {
    ...
    a.fn.iCheckbox.methods = {
        getValue: function(a) {
            ...
            return b.each(function() {
                var a = c[this.name];
                return a ? (a.sort || (c[this.name] = [a]),
                void c[this.name].push(this.value)) : void (c[this.name] = this.value)
            }),
            c
        },
        check: function(b, d) {
            ...
            a.ajax({
                ...
                success: function(a) {
                    ...
                    for (i = b.length - 1; i >= 0; i--)
                        g = d.combotree("tree").iTree("find", b[i].replace(/'/g, "")),
    ...
    a.fn.iCombotree.defaults = {
        ...
        onBeforeSelect: function(b) {
            ...
            if (c.onlyLeafCheck) {
                ...
                if (!d)
                    return a.messager.alert("提示操作！", "请展开选择子节点！", "warning"),
                    !1
            }
        }
    }
            ...
            d.getFatherIdsUrl && setTimeout(function() {
                ...
                if (null == b && "" != c.id) {
                    ...
                    a.ajax({
                        ...
                        success: function(a) {
                            for (var b = a.split(","), c = b.length - 1; c >= 0; c--)
                                g = f.treegrid("find", b[c].replace(/'/g, "")),
...
var openDialog = function(a) {
    ...
    if ("object" == typeof d)
        ...
        var e = $("#" + c.id);
        if (e.iDialog(b.dialog),
        b.dialog.href.indexOf("{") != -1) {
  ...
  , addParentTab = function(a) {
    ...
    if ("object" == typeof d.grid) {
        ...
        if (1 == f.checkboxSelect) {
            ...
            if (0 == g.length)
                return void $.messager.alert(topJUI.language.message.title.operationTips, topJUI.language.message.msg.checkSelfGrid, topJUI.language.message.icon.warning);
  ...
  , openWindow = function(a) {
    ...
    if ("object" == typeof c.grid)
        if (1 == c.grid.checkboxSelect) {
...
function(a) {
        ...
        if (!e) {
            ...
            e = this.headerContextMenu = h.menu({
                onClick: function(b) {
                    ...
                    b.iconCls == f ? (d.datagrid("hideColumn", c),
                    a(this).menu("setIcon", {
                        target: b.target,
                        iconCls: g
                    })) : (d.datagrid("showColumn", c),
                    a(this).menu("setIcon", {
                        target: b.target,
                        iconCls: f
                    })),
    ...
    a.fn.iDatagrid.methods = {
        ...
        singleSelectedAjax: function(b, c) {
            ...
            return null == d ? void a.messager.alert(topJUI.language.message.title.operationTips, topJUI.language.message.msg.selectSelfGrid, topJUI.language.message.icon.warning) : (c.url = replaceUrlParamValueByBrace(c.url, d),
            void a.messager.confirm(topJUI.language.message.title.confirmTips, c.confirmMsg, function(a) {
                if (a) {
                    var b = doAjax(c);
        ...
        cancelCellTip: function(b) {
            return b.each(function() {
                ...
                b.tipDelayTime && (clearTimeout(b.tipDelayTime),
                b.tipDelayTime = null)
            })
        }
    },
    ...
    a.fn.iDatagrid.defaults = {
        ...
        onLoadSuccess: function() {
            ...
            "object" == typeof b.childGrid)
                for (var c = b.childGrid.grids, d = 0; d < c.length; d++) {
        ...
        onClickRow: function(b, c) {
            ...
            if ("object" == typeof d.childTabs)
                for (var h = d.childTabs, g = 0; g < h.length; g++) {
                    var i = a("#" + h[g].id)
                      , j = i.tabs("options")
                      , k = i.tabs("getSelected")
                      , l = k.panel("options");
                    if (null != l.href || null != l.iframeHref) {
                        null != l.href && (l.tempHref = l.href,
                        l.iframeHref = l.href,
                        l.href = null),
                        ...
                        "object" == typeof l.dgOpts && ("object" == typeof j.parentGrid && setGridQueryParams(j.parentGrid, "datagrid", l.dgOpts.id),
                        a("#" + l.dgOpts.id).datagrid("reload"))
                }
        },
    ...
    a.fn.iDialog.methods = {
        createDialog: function(b, c) {
            if (0 == a("#" + c.id).length) {
                ...
                if (a("body").append(e + '<div id="' + c.id + '-buttons" style="display:none">' + f + '<a href="#" class="closeDialog" onclick="javascript:$(\'#' + c.id + "').dialog('close')\">关闭</a></div>"),
                "object" == typeof c.buttonsGroup) {
    ...
    a.fn.iDialog.defaults = {
        ...
        onLoad: function() {
            ...
            if (b.mbId) {
                ...
                if (a.messager.progress("close"),
                a(this).trigger(topJUI.eventType.initUI.form),
                a(this).dialog("center"),
                void 0 != b.url) {
                    var e = getSelectedRowData(d.grid.type, d.grid.id)
                      , f = replaceUrlParamValueByBrace(b.url, e);
                    a.getJSON(f, function(d) {
                        if (c.form("load", d),
                        "string" == typeof b.editor || "object" == typeof b.editor) {
                            if ("string" == typeof b.editor) {
                                var e = []
                                  , f = [];
                                e = b.editor.replace(/'/g, '"').split(",");
                                for (var g = 0; g < e.length; g++)
                                    f.push(strToJson(e[g]))
                            } else
                                f = b.editor;
...
$.extend($.fn.datagrid.methods, {
    fixRownumber: function(a) {
        return a.each(function() {
            ...
            c > 25 ? ($(".datagrid-header-rownumber,.datagrid-cell-rownumber", a).width(c + 5),
            $(this).datagrid("resize"),
            b.remove(),
            b = null) : $(".datagrid-header-rownumber,.datagrid-cell-rownumber", a).removeAttr("style")
        })
    },
    addToolbarItem: function(jq, items) {
        return jq.each(function() {
            ...
            toolbar.length || (toolbar = $('<div class="datagrid-toolbar"><table cellspacing="0" cellpadding="0"><tr></tr></table></div>').prependTo(dpanel),
            $(this).datagrid("resize"));
    ...
    removeToolbarItem: function(a, b) {
        return a.each(function() {
            ...
            d && d.length > 0 && (d.closest("td").remove(),
            d = null)
        })
    }
}),
        ...
        onClick: function() {
            ...
            if ("ajaxForm" == b.handler || "multiAjaxForm" == b.handler) {
                ...
                if (a("#" + b.dialogId).form("validate")) {
                    if (b.ajaxData = a("#" + b.dialogId).serialize(),
                    void 0 != b.combotreeFields) {
                        var f = "";
                    ...
                    if ("multiAjaxForm" == b.handler) {
                        ...
                        if (0 == g.length)
                            return void a.messager.alert(topJUI.language.message.title.operationTips, topJUI.language.message.msg.checkSelfGrid, topJUI.language.message.icon.warning);
                        var h = void 0 == e.pkName ? topJUI.config.pkName : e.pkName;
                        b.ajaxData += "&" + h + "s=" + getMultiRowsFieldValue(g, h)
                    }
                    var i = doAjax(b);
                    if (200 == i.statusCode) {
                        if (a("#" + b.dialogId).dialog("close"),
                        "object" == typeof e)
                            if ("datagrid" == e.type)
                                a("#" + e.id).datagrid("reload");
...
function(a) {
    a.fn.iNumberspinner = function(b, c) {
        ...
        this.each(function() {
            ...
            "currentYear" == b.defaultValueType ? (b.value = currentYear,
            b.min = 1900,
            b.max = 2200) : "currentSeason" == b.defaultValueType ? (1 == currentMonth || 2 == currentMonth || 3 == currentMonth ? b.value = 1 : 4 == currentMonth || 5 == currentMonth || 6 == currentMonth ? b.value = 2 : 7 == currentMonth || 8 == currentMonth || 9 == currentMonth ? b.value = 3 : 10 != currentMonth && 11 != currentMonth && 12 != currentMonth || (b.value = 4),
            b.min = 1,
            b.max = 4) : "currentMonth" == b.defaultValueType ? (b.value = currentMonth,
            b.min = 1,
            b.max = 12) : "currentDay" == b.defaultValueType ? (b.value = currentDay,
            b.min = 1,
            b.max = 31) : "currentHour" == b.defaultValueType && (b.value = currentHour,
            b.min = 0,
            b.max = 24),
...
+function(a) {
    ...
    a(document).on(topJUI.eventType.initUI.form, function(b) {
        ...
        a('[data-toggle="topjui-upload-ue"]').each(function(b) {
            ...
            g.ready(function() {
                ...
                var b = "afterConfirmUploadedFile"
                  , e = "url";
                "image" == d.uploadType && (b = "afterConfirmUploadedImage",
                e = "src"),
        ...
        a('[data-toggle="topjui-upload-lay"]').each(function(b) {
            var c = a(this)
              ...
              , d = a.extend(e, d);
            d = setId(c, d),
            0 == a("#" + d.id + "-btn").length && (a("body").append('<button type="button" class="layui-btn" id="' + d.id + '-btn" style="display: none;">上传图片</button>'),
            layui.use("upload", function() {
                var a = layui.jquery
                  , b = layui.upload;
                b.render({
    ...
    a(document).on(topJUI.eventType.initUI.base, function(b) {
        ...
        a('[data-toggle="topjui-uploader"]').each(function() {
            ...
            d.on("uploadSuccess", function(b) {
                for (var c = e.datagrid("getRows"), d = 0; d < c.length; d++)
                    c[d].fileId == b.id && (a('input[type="checkbox"]')[d + 1].disabled = !0);
            ...
            var k = function() {
                for (var a = e.datagrid("getSelections"), b = [], c = 0; c < a.length; c++)
                    b.push(a[c]);
            ...
            b.on("click", function() {
                if ("object" == typeof c.parentGrid && (f = getSelectedRowData(c.parentGrid.type, c.parentGrid.id),
                !f))
                    return void a.messager.alert(topJUI.language.message.title.operationTips, c.parentGrid.unselectedMsg || topJUI.language.message.msg.selectParentGrid, topJUI.language.message.icon.warning);
                var b = e.datagrid("getRows");
                b.length > 0 && (e.datagrid("selectAll"),
                k());
    ...
    a(document).on(topJUI.eventType.initUI.advanceSearchForm, function(b) {
        for (var c = a.cookie("fieldNameStr").split(","), d = a.cookie("colNameStr").split(","), e = [], f = 0; f < d.length; f++)
            e.push({
                value: c[f],
                text: d[f]
            });
    ...
    a(document).on(topJUI.eventType.initUI.importExcelForm, function(b) {
        setTimeout(function() {
            for (var b = a.cookie("fieldNameStr"), c = b.split(","), d = "", e = 0; e < c.length; e++)
                d += e == c.length - 1 ? "'{" + e + "}'" : "'{" + e + "}',";
    ...
    a.fn.iTabs.defaults = {
        ...
        onSelect: function(b, c) {
            ...
            if (null != g.href || null != g.iframeHref) {
                if (null != g.href && (g.tempHref = g.href,
                g.iframeHref = g.href,
                g.href = null),
                "object" == typeof e && (g.iframeHref = setPanelQueryParams(g.tempHref, e)),
                g.iframeHref.indexOf("{") == -1) {
                ...
                i.length > 0 ? a("#" + g.id).children('[data-toggle="topjui-datagrid"]').each(function(b) {
                    ...
                    h.datagrid("load"),
                    g.dgOpts = d
                }) : "object" == typeof g.dgOpts && ("object" == typeof e && setGridQueryParams(e, "datagrid", g.dgOpts.id),
                a("#" + g.dgOpts.id).datagrid("reload")),
        ...
        this.each(function() {
            ...
            for (var c = 0; c < b.list.length; c++) {
                ...
                for (var d = 0; d < b.list[c].list.length; d++) {
                    ...
                    for (var f = 0; f < b.list[c].list[d].more.length; f++)
                        a("ul .more" + c + d).append("<p>" + b.list[c].list[d].more[f] + "</p>")
                }
            }
        }),
        a(function() {
    ...
    a.fn.iTree.defaults = {
        ...
        onClick: function(b) {
            ...
            if ("postCodeItemIdAndRefreshDatagrid" == c.clickEvent) {
                ...
                "closed" == b.state ? a(c.id).tree("expand", b.target) : a(c.id).tree("collapse", b.target)
        },
        onLoadSuccess: function() {
...
function(a) {
    ...
    a.fn.iTreegrid.methods = {
        cascadeCheck: function(b, c) {
            ...
            if (!f.singleSelect) {
                for (var g = f.idField, h = !1, i = a(b.selector).treegrid("getSelections"), j = 0; j < i.length; j++)
                    i[j][g] == c.id && (h = !0);
                d(b, c.id, g, h),
    ...
    a.fn.iTreegrid.defaults = {
        ...
        onClickRow: function(b) {
            ...
            if ("object" == typeof c.childTabs)
                for (var j = c.childTabs, f = 0; f < j.length; f++) {
                    var k = a("#" + j[f].id)
                      , l = k.tabs("options")
                      , m = k.tabs("getSelected")
                      , n = m.panel("options");
                    if (null != n.href || null != n.iframeHref) {
                        void 0 != n.href && (n.iframeHref = n.href,
                        n.href = null);
                        ...
                        "object" == typeof n.dgOpts && ("object" == typeof l.parentGrid && setGridQueryParams(l.parentGrid, "datagrid", n.dgOpts.id),
                        a("#" + n.dgOpts.id).datagrid("reload"))
                }
        },
