package com.profound.common.render;
...
public class CaptchaRender extends Render {
	...
	public void render() {
		...
		request.getSession().setAttribute(captchaName, captcha.getKey());
	...
	protected String getCaptchaKeyFromSession() {
		String captchaKey = null;
		if (request.getSession().getAttribute(captchaName) != null) {
			captchaKey = request.getSession().getAttribute(captchaName).toString();
		}
		return captchaKey;
	}
	 ...
	 */
	public static boolean validate(Controller controller, String userInputString,boolean clear) {
		String captchaKey = controller.getCookie(captchaName);
		if (validate(captchaKey, userInputString,clear)) {
			if(clear){
	 ...
	 * @param controller
	 */
	public static void clearCaptchaCookie(Controller controller) {
		ICaptchaCache captchaCache = CaptchaManager.me().getCaptchaCache();
		Captcha captcha = captchaCache.get(controller.getCookie(captchaName));
	 ...
	 * @return 验证通过返回 true, 否则返回 false
	 */
	public static boolean validate(String captchaKey, String userInputString,boolean clear) {
		ICaptchaCache captchaCache = CaptchaManager.me().getCaptchaCache();
